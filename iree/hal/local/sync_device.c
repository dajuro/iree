...
#include "iree/hal/local/sync_command_buffer.h"
#include "iree/hal/local/sync_event.h"
#include "iree/hal/local/sync_queue.h"
...
static iree_status_t iree_hal_task_device_queue_submit(
  ...
  for(unsigned int current_batch=0; current_batch < batch_count; ++current_batch){
    const iree_hal_submission_batch_t* batch = &batches[current_batch];

    for(unsigned int current_cmd_buffer=0; current_cmd_buffer < batch->command_buffer_count; ++current_cmd_buffer){
      iree_hal_command_buffer_t* cmd_buffer = batch->command_buffers[current_cmd_buffer];
      iree_hal_task_command_buffer_execute(cmd_buffer, base_device);
    }
  }
  return iree_ok_status();
