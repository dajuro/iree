...
iree_status_t iree_hal_task_command_buffer_execute(
    iree_hal_command_buffer_t* base_command_buffer,
    iree_hal_device_t* device) {
  iree_hal_task_command_buffer_t* command_buffer =
      iree_hal_task_command_buffer_cast(base_command_buffer);
  ...
  for(iree_host_size_t i=0; i < iree_task_list_calculate_size(&command_buffer->root_tasks); ++i){
    iree_task_t* task = iree_task_list_pop_front(&command_buffer->root_tasks);
